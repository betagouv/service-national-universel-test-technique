...
const { ActivityModel, ProjectModel } = require("../models");
...
router.get("/", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    ...
    const data = await ActivityModel.find({ ...query, organisation: req.user.organisation }).sort("-created_at");
...
router.post("/", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    const body = req.body;
    await ProjectModel.findOneAndUpdate({ _id: body.projectId }, { last_updated_at: new Date() }, { new: true });
    const query = { projectId: body.projectId, userId: body.userId, date: body.date };
    const activities = await ActivityModel.findOneAndUpdate(query, { ...body, organisation: req.user.organisation }, { new: true, upsert: true });
